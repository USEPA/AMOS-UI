  ...
  export default {
    ...
    methods: {
      createStripPlot() {
        ...
        async function stripPlotsMain(data) {
          // read in data
          ...
          function makeStripPlot(data, sortedBy, showMode = "both", firstPass = false, zoom) {
            ...
            svgIDs.forEach((svgID, iPlot) => {
              ...
              const gridGroup = g.append("g")
                .attr("class", "strip-plot-grid-lines");
              
              gridGroup.selectAll(".strip-plot-y-grid")
                .data(yScale.domain())
                .enter()
                .append("line")
                .attr("class", "strip-plot-y-grid")
                .attr("x1", margin.left)
              ...
              gridGroup.selectAll(".strip-plot-x-grid")
                .data(xTicks.slice(0, xTicks.length - 2))
                .enter()
                .append("line")
                .attr("class", "strip-plot-x-grid")
                .attr("x1", d => xScale(d))
                .attr("x2", d => xScale(d))
                .attr("y1", margin.top)
        ...
        // extract sheet and generate visualization
        const sheetName = "Surrogate Detection Statistics";
        const json_data = XLSX.utils.sheet_to_json(this.workbook.Sheets[sheetName]);
        stripPlotsMain(json_data)
      }
    }
  }
</script>