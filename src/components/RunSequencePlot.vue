...
<script>
  ...
  export default {
    ...
    mounted() {
       ...
       */
      async function readInterpretOutputXLSX(workbook) {
        // fetch file
       ...
       */
      async function readInterpretSequenceCSV(csv_string) {
        // fetch the file
        //const response = await fetch(filePath);
      ...
      function makeTooltip() {
        const tooltipContainer = d3.select("div#legendTooltip-grid-container") //changed
          .append("div")
       ...
       * @param {Object} workbook Workbook object, generated by the XLSX library and passed from the parent component.
       * @param {string} csv_string CSV version of the sheet that has the sequence mapping.
       */
      async function mainRunSequence(workbook, seqPath) {
        var linesOfBestFit = true;
        var yScaleType = "log";

        // get data
        let [ dataPos, dataNeg ] = await readInterpretOutputXLSX(workbook);
      ...
      /* const xlsxPath = "http://127.0.0.1:5173/src/data/TM_7ppm_10MRL_NTA_WebApp_results.xlsx";
      const seqCsvPath = "http://127.0.0.1:5173/src/data/WW2DW_sequence-jon.csv";
      mainRunSequence(xlsxPath, seqCsvPath); */

      const worksheet_names = this.workbook.SheetNames
      let mapping_worksheet = null
      if (!worksheet_names.includes("Run Sequence (pos)")) {
        mapping_worksheet = this.workbook.Sheets["Run Sequence (neg)"]
      } else if (!worksheet_names.includes("Run Sequence (neg)")) {
        mapping_worksheet = this.workbook.Sheets["Run Sequence (pos)"]
      } else {
        const positive_map = XLSX.utils.sheet_to_json(this.workbook.Sheets["Run Sequence (pos)"])
        const negative_map = XLSX.utils.sheet_to_json(this.workbook.Sheets["Run Sequence (neg)"])

        const full_map = positive_map.concat(negative_map)
        mapping_worksheet = XLSX.utils.json_to_sheet(full_map)
      }
      
      const mapping_csv_string = XLSX.utils.sheet_to_csv(mapping_worksheet)

      mainRunSequence(this.workbook, mapping_csv_string)
