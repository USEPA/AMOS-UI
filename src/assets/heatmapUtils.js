...
function getTargetBoundsFromZoomBox(zoomBox, camera, cameraDefaults) {
  ...
  const ndcX0 = (zoomBox.x0 / Math.abs(cameraDefaults.left - cameraDefaults.right))*2 - 1;
  const ndcY0 = (zoomBox.y0 / Math.abs(cameraDefaults.top - cameraDefaults.bottom))*2 + 1;
  const ndcX1 = (zoomBox.x1 / Math.abs(cameraDefaults.left - cameraDefaults.right))*2 - 1;
  const ndcY1 = (zoomBox.y1 / Math.abs(cameraDefaults.top - cameraDefaults.bottom))*2 + 1;
...
export function getGeometries(dimsObject) {
  const graphGeometry = new THREE.PlaneGeometry(dimsObject.width, dimsObject.height);
  const cellGeometry = new THREE.PlaneGeometry(dimsObject.apparentCellWidth, dimsObject.cellHeight);
  const horzLineGeo = new THREE.PlaneGeometry(dimsObject.width, 0.5);
  const vertLineGeo = new THREE.PlaneGeometry((dimsObject.cellWidth / 10), dimsObject.height);
 ...
 */
export function getMaterials(failC=0xff0000, nonDetectC=0xc7c8c9, passC=0xffffff, zoomBoxColor=0x000000) {
  ...
  const clearMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff, opacity: 0, transparent: true });
  const blackMaterial = new THREE.MeshBasicMaterial({ color: 0x000000 });
  const zoomBoxMaterial = new THREE.LineBasicMaterial({ color: zoomBoxColor }); // 0x00ffff

  return [redMaterial, greyMaterial, whiteMaterial, clearMaterial, zoomBoxMaterial, blackMaterial];
}
 ...
 */
export function setCellColorAndPos(cvDataFlat, dimsObject, greyMesh, redMesh, whiteMesh) {
  ...
  cvDataFlat.forEach((cell, index) => {
    ...
    y += -(cell.sampleIndex * dimsObject.cellHeight) + (dimsObject.cellHeight / 2); // shift by row and center to row
    ...
    if (cell.value === -1) {
      ...
      cell.cvIndex = index;
      redCellInstances.push({ "index": redIndex, "scaleX": 1, "scaleY": 1, "x": x, "y": y}) // for animating red cells
      redIndex++;
 ...
 */
export function addYAxisLabelsAndHorzLines(canvas, sampleGroups, dimsObject, horzLineGeo, blackMaterial, graphMesh, scene) {
  ...
  sampleGroups.forEach((header, index) => {
    ...
    const lineStartX = dimsObject.paddingWidth + (dimsObject.width / 2) - (dimsObject.actualWidth / 2);
    const lineY = -(dimsObject.height / 2) + (dimsObject.cellHeight * (index+1));
  ...
  const lineStartX = dimsObject.paddingWidth + (dimsObject.width / 2) - (dimsObject.actualWidth / 2);
  const lineY = -(dimsObject.height / 2) + (dimsObject.cellHeight * (sampleGroups.length + 1));
 ...
 */
export function getVertLines(dimsObject, nFeatures, vertLineGeo, blackMaterial) {
 ...
 */
export function clickTitleEvent(event, redCellInstances, redMesh, redCellZoomed) {
  ...
  if (event.ctrlKey){
    ...
    redCellInstances.forEach(instance => {
      ...
      const redCellTweenIn = new TWEEN.Tween({ scaleX: instance.scaleX, scaleY: instance.scaleY, rotX: 0 })
        .to({ scaleX: instance.scaleX*30, scaleY: instance.scaleY*2, rotX: Math.PI }, 800)
        .onUpdate(updated => {
        ...
        const redCellTweenOut = new TWEEN.Tween({ scaleX: instance.scaleX*30, scaleY: instance.scaleY*2, rotX: Math.PI })
        .to({ scaleX: instance.scaleX, scaleY: instance.scaleY, rotX: 2*Math.PI }, 800)
        .onUpdate(updated => {
          dummy.position.set(instance.x, instance.y, 0);
          dummy.scale.set(updated.scaleX, updated.scaleY, 1);
          dummy.rotation.set(0, 0, updated.rotX)
  ...
  } else { // if ctrl not held, toggle red cell width increase
    if (!redCellZoomed) { // "zoom" into red cells
      redCellInstances.forEach(instance => {
        const redCellTween = new TWEEN.Tween({ scaleX: instance.scaleX, scaleY: instance.scaleY, rotX: 0 })
          .to({ scaleX: instance.scaleX*22, scaleY: instance.scaleY, rotX: 0 }, 400)
          .onUpdate(updated => {
            dummy.position.set(instance.x, instance.y, 0);
            dummy.scale.set(updated.scaleX, updated.scaleY, 1);
            dummy.rotation.set(0, 0, updated.rotX)
      ...
      redCellZoomed = !redCellZoomed;
    } else { // "zoom" out of red cells
      redCellInstances.forEach(instance => {
        const redCellTween = new TWEEN.Tween({ scaleX: instance.scaleX*22, scaleY: instance.scaleY, rotX: 0 })
          .to({ scaleX: instance.scaleX, scaleY: instance.scaleY, rotX: 0 }, 400)
          .onUpdate(updated => {
            dummy.position.set(instance.x, instance.y, 0);
            dummy.scale.set(updated.scaleX, updated.scaleY, 1);
            dummy.rotation.set(0, 0, updated.rotX)
 ...
 */
export function mouseenterYAxisLabelEvent(event, label, samplePassCounts, yAxisTooltip, dimsObject) {
  
  const sampleName = label.innerHTML;
  const sampleData = samplePassCounts[sampleName];
  
  if (sampleData) {
    yAxisTooltip.innerHTML = `<div style="background-color: white; color: black; padding: 5px; border-radius: 3px; border: solid 1px white;"><b>Sample Name</b>: ${sampleName}</div>\n<div style="margin-top: -5px"><b>Pass</b>: ${sampleData["nPass"].toLocaleString()}\n<b>Fail</b>: <span style="color: rgb(255, 160, 160);">${sampleData["nFail"].toLocaleString()}</span>\n<b>Non-Detect</b>: <span style="color: rgb(200,200,200)">${sampleData["nNonDetect"].toLocaleString()}</span></div>`;
...
export function mousedownCellEvent(event, startX, startY, zoomBox, zoomBoxGeometry, zoomed) {
...
export function mousemoveCellEvent(
  ...
  if (intersects.length > 0) {
    ...
    if (intersectedObject === redMesh) {
      cellData = cvDataFlat.find((cell, i) => cell.color === "red" && cell.meshIndex === instanceId);
    } else if (intersectedObject === whiteMesh) {
      cellData = cvDataFlat.find((cell, i) => cell.color === "white" && cell.meshIndex === instanceId);
    } else if (intersectedObject === greyMesh) {
      cellData = cvDataFlat.find((cell, i) => cell.color === "grey" && cell.meshIndex === instanceId);
    }
  ...
  if (zoomBox) {
    ...
    const vertices = new Float32Array([
      v0[0], v0[1], v0[2],   // top-left
      v1[0], v1[1], v1[2],   // top-right
      v2[0], v2[1], v2[2],   // bottom-right
      v3[0], v3[1], v3[2],   // bottom-left
      v0[0], v0[1], v0[2]    // top-left (close)
    ]);

    zoomBoxGeometry.setAttribute('position', new THREE.BufferAttribute(vertices, 3));
...
export function mouseupCellEvent(
  ...
  if (zoomBoxObject) {
    ...
    // update camera
    let targetBounds = getTargetBoundsFromZoomBox(zoomBox, camera, cameraDefaults);

    zoomTween(scene, camera, targetBounds, orbitControls, vertLineObjects, vertLineLimit, dimsObject, null);
    ...
    if (redCellZoomed) {
      ...
      redCellInstances.forEach(instance => {
        const redCellTween = new TWEEN.Tween({ scaleX: instance.scaleX*22, scaleY: instance.scaleY, rotX: 0 })
          .to({ scaleX: instance.scaleX, scaleY: instance.scaleY, rotX: 0 }, 100)
          .onUpdate(updated => {
            dummy.position.set(instance.x, instance.y, 0);
            dummy.scale.set(updated.scaleX, updated.scaleY, 1);
            dummy.rotation.set(0, 0, updated.rotX)
...
export async function keydownDocEvent(
  ...
  if (event.ctrlKey && event.code === 'Space') {
    if (zoomed) {
      ...
      await zoomTween(scene, camera, targetBounds, orbitControls, vertLineObjects, vertLineLimit, dimsObject, null, true);
      zoomed = !zoomed;
      ...
      // unzoom camera
      let targetBounds = getTargetBoundsFromZoomBox(cachedZoomBox, camera, cameraDefaults);
      await zoomTween(scene, camera, targetBounds, orbitControls, vertLineObjects, vertLineLimit, dimsObject, cachedOrbitControl);
      zoomed = !zoomed;
      graphMesh.visible = false;
    }
  }
