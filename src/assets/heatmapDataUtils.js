...
export function getUniqueHeaders(dataArr) {
  ...
  for (let rowObj of data) { // iterate over rows
    Object.keys(rowObj).forEach(key => { // iterate over column headers
...
export function getUniqueSampleHeaders(dataArr) {
  ...
  for (let sublist of allHeaders) {
    if ((sublist.length > 1) && (!non_samples.some(x => sublist[0].includes(x)))) {
  ...
  const sampleGroups = samHeaders.filter(item => 
    !prefixes.some(prefix => item.includes(prefix))
  );
  ...
  cvDataDiscrete.forEach((feature, featureIndex) => {
    ...
    Object.entries(feature).forEach(([sample, value], k) => {
      ...
      if (!sample.endsWith("tureID")) {
        ...
        const passFail = value === 1 ? "Fail" : value === 0 ? "Pass" : "Non-Detect";
        cvDataFlat.push({
          "featureIndex": featureIndex,
          "sampleIndex": sampleIndex,
          "value": value,
          "color": passFail === "Pass" ? "white" : passFail === "Fail" ? "red" : "grey",
          "sampleName": s_name,
...
export function getSamplePassCounts(cvDataFlat, nFeatures) {
  ...
  cvDataFlat.forEach((item, index) => {
    ...
    // add counts
    const color = item["value"] === 1 ? "red" : item["value"] === 0 ? "white" : "grey";
    if (color === "white") {
  ...
  samplePassCounts["total"]["nOccurrences"] = samplePassCounts["total"]["nFeatures"] * samplePassCounts["total"]["nSamples"];
