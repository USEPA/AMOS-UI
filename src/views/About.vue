...
<template>
  <div class="About">
    <h1>About</h1>
    <p>This web application is designed to perform searches on and allow examination of a collection of publicly
    ...
    <ul>
      <li><b>Spectra</b> of various types. These are primarily mass spectra, with smaller amounts of NMR and IR spectra.
        Some spectra are stored as PDFs that may contain some supplemental information, and some others are merely
        linked to and not stored directly in this database.
      </li>
      <li><b>Analytical methods</b> that document full testing procedures for an analysis, including information like
        sample matrices, limits of detection, etc. These usually cover several substances, but a few may cover as many
        as a few hundred.
      </li>
      <li><b>Fact sheets</b>, which are supplemental documents of various kinds. These range from brief, relatively
        non-technical descriptions of a substance to monographs that detail multiple experimental spectra.
      </li>
    </ul>
    <p>The primary functions of this app are as follows:</p>
    <ul>
      <li>Performing general searches on all categories of records for specfic substances, and displaying information
        about those records. Searching can be done by DTXSID, CAS registry number, InChIKey, or substance names. Some
        searches can be done on partial identifiers, i.e., identifiers that aren't unique to one substance.
      </li>
      <li>Viewing lists of fact sheets and methods in the database, along with displaying the documents themselves and
        the substances associated with them.
      </li>
      <li>Searching for all methods and fact sheets that cover a substance similar to one being searched. This uses EPA
        calculations for the similarity between two substances and returns all methods and fact sheets containing at
        least one substance of sufficient similarity (similarity level is currently hardcoded on the Flask backend).
        Searching can be done by DTXSID, CAS registry number, InChIKey, or name.
      </li>
      <li>Batch searching substances by DTXSID, and returning basic information for those records.</li>
      <li>Comparing a user-submitted spectrum to other spectra in the database, given an analytical methodology
        (currently just GC/MS or LC/MS) and mass range.
      </li>
    </ul>
    <p>The records in this application contain:</p>
    <ul v-if="summary_retrieved">
      <li>Spectra in database: {{ internal_spectra["Total"].toLocaleString() }} spectra total,
        {{ internal_spectra["PDF"].toLocaleString() }} of which are stored as PDFs, covering
        {{ substances["Internal Spectrum"].toLocaleString() }} substances. Among those that are not PDFs, there are:
      </li>
      <ul>
      ...
      <li>Externally linked spectra: {{ external_spectra_count.toLocaleString() }} links for spectra of various kinds,
        covering {{ substances["External Spectrum"].toLocaleString() }} substances.
      </li>
      <li>Fact Sheets: {{ fact_sheet_count.toLocaleString() }} documents, covering
        {{ substances["Internal Fact Sheet"].toLocaleString() }} substances.
      </li>
      <li>Methods: {{ method_count.toLocaleString() }} documents, covering
        {{ substances["Internal Method"].toLocaleString() }} substances.
      </li>
...
import {BACKEND_LOCATION, store} from '@/assets/store.js'
